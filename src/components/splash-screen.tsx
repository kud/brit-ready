...
// Pick one message per load, cached so render stays pure.
let messageIndex = -1;
const getMessage = () => {
  if (messageIndex < 0) messageIndex = Math.floor(Math.random() * messages.length);
...
export const SplashScreen = () => (
  ...
  >
    ...
    <div className="text-center">
      ...
      <p className="mt-2 text-sm text-muted">{getMessage()}</p>
    </div>
